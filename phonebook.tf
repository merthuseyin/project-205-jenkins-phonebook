...
resource "aws_instance" "docker-machine-managers" {
  ...
  tags = {
    Name = "mert-Docker-Swarm-Manager-${count.index + 1}"
...
resource "aws_instance" "docker-machine-workers" {
  ...
  tags = {
    Name = "mert-Docker-Swarm-Worker-${count.index + 1}"
